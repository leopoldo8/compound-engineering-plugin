...
4. `/ce-code-review mode:report-only plan:<plan-path-from-step-2>` — spawn as background Task agent
5. `/ce-test-browser` — spawn as background Task agent

Wait for both to complete before continuing.

## Autofix Phase

6. `/ce-code-review mode:autofix plan:<plan-path-from-step-2>` — run sequentially after the parallel phase so it can safely mutate the checkout, apply `safe_auto` fixes, and emit residual todos for step 7

## Finalize Phase

7. `/ce-todo-resolve` — resolve findings, compound on learnings, clean up completed todos
8. `/ce-feature-video` — record the final walkthrough and add to PR
